...
private:
    ...
    std::vector<double> CalculateAnotherMultiplier (double j, const std::size_t maxL, double sintheta) {
    ...
    std::vector<complex> CalculateFirstMultiplier(const std::size_t maxL, double sintheta) {
        //for (std::size_t l=0; l <= maxL; l++) Y_v_r[YR(l,0)] =P_v[PT(l,0)];
...
public:
    ...
    void calculate(std::size_t maxL) {
       ...
       for (int j=2; j<=numofArguments; j++) {
          multipliers.push_back(CalculateAnotherMultiplier(j, maxL, Theta[j-1])); //j!=1 real number
       }
...
//std::cout << vec.multiplier[1].r << " * " << vec.multipliers[0][PT(1,1)] << " * " << vec.multipliers[1][PT(1,1)];
return 0;
}
