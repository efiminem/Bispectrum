...
P_v[PT(l-2,m)]);
    ...
    void computeY (const std::size_t L, double y) {
        for (std::size_t l=0; l <= L; l++) Y_v[YR(l,0)] = 
P_v[PT(l,0)]*0.5*tab.SQRT2;
        ...
        for (std::size_t m =1; m <= L; m++) {
            ...
            for (std::size_t l = m; l <= L; l++) {
                Y_v[YR(l,-m)]=P_v[PT(l,m)]*s;
                Y_v[YR(l,m)]=P_v[PT(l,m)]*c;
            }
        }
    }
public:
    ...
    Y_v = std::vector<double>(YR(maxL, maxL)+1,0);
    tab.update(maxL);
    computeALF(maxL, Theta[0]);
    computeY(maxL, Theta[1]);
    }
    double Y (int l, int m) {return Y_v[YR(l,m)];}
